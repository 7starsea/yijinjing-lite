...
{
    spdlog::info( "[RmPage] (folder) {} (jname) {} (pNum) {} (lpNum) {}",  msg.folder,  msg.name,  msg.page_num, msg.last_page_num);
...
void  PageEngine::exit_client(const string& clientName, int hashCode, bool needHashCheck)
{
    ...
    spdlog::info( "[RmClient] (name) {} (start) {} (end) {}", clientName, info.reg_nano, getNanoTime());
